function SongCard({ song, isActive, onSelect, compact = false }) {
  return (
    <button
      type="button"
      onClick={() => onSelect?.(song)}
      className={[
        `group relative w-full cursor-pointer overflow-hidden rounded-3xl border bg-[linear-gradient(180deg,rgba(255,255,255,0.1),rgba(255,255,255,0.04))] text-left shadow-[0_18px_45px_rgba(0,0,0,0.28)] transition duration-300 hover:-translate-y-1.5 hover:scale-[1.02] hover:border-[#1DB954]/40 hover:shadow-[0_28px_65px_rgba(29,185,84,0.18)] ${compact ? 'p-4' : 'p-5'}`,
        isActive
          ? 'border-[#1DB954]/50 bg-[linear-gradient(180deg,rgba(29,185,84,0.16),rgba(255,255,255,0.04))]'
          : 'border-white/8',
      ].join(' ')}
    >
      ...
      <div className={`relative flex h-full ${compact ? 'gap-3' : 'gap-4'}`}>
        <img
          src={song.coverArt}
          alt={`${song.title} cover art`}
          className={`${compact ? 'h-20 w-20' : 'h-24 w-24'} shrink-0 rounded-2xl object-cover shadow-[0_12px_24px_rgba(0,0,0,0.28)]`}
          loading="lazy"
        />

        <div className={`flex min-w-0 flex-1 flex-col justify-between ${compact ? 'gap-3' : 'gap-4'}`}>
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <h3
                className={`mt-2 overflow-hidden text-ellipsis whitespace-nowrap font-semibold tracking-tight text-white ${compact ? 'text-base' : 'text-lg'}`}
              >
                {song.title}
              </h3>
          ...
          <div className={`rounded-2xl border border-white/8 bg-black/20 ${compact ? 'p-2.5' : 'p-3'}`}>
          ...
          <div className={`rounded-2xl border border-white/8 bg-white/[0.03] ${compact ? 'p-2.5' : 'p-3'}`}>
            <p className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">歌曲介紹</p>
            <p
              className={`mt-2 text-zinc-300 ${compact ? 'line-clamp-3 text-[13px] leading-5' : 'line-clamp-4 text-sm leading-6'}`}
            >
              {song.summary}
            </p>
          </div>
