...
os.environ['JAZZ_SKIP_BOOTSTRAP'] = '1'
import SQL

SCHEMA_SQL = """USE JazzDB;
GO
...
def main():
    model = SQL.load_relational_model()

    schema_path = Path('JazzDB_schema.sql')
    seed_path = Path('JazzDB_seed.sql')
    full_path = Path('JazzDB_full_import.sql')

    seed_sections = []
    seed_sections.extend(SQL.build_insert_batches('Artists', ['artist_id', 'name', 'country'], model['artists']))
    seed_sections.extend(SQL.build_insert_batches('Albums', ['album_id', 'title', 'release_year'], model['albums']))
    seed_sections.extend(SQL.build_insert_batches('Playlists', ['playlist_id', 'playlist_name', 'owner_id', 'owner_name', 'total_followers'], model['playlists']))
    seed_sections.extend(SQL.build_insert_batches('Songs', ['song_id', 'title', 'artist_id', 'tempo', '[key]', 'genre', 'duration', 'popularity', 'release_year', 'decade'], model['songs'].rename(columns={'key': '[key]'})))
    seed_sections.extend(SQL.build_insert_batches('Song_Album', ['song_id', 'album_id'], model['song_album']))
    seed_sections.extend(SQL.build_insert_batches('Audio_Features', ['song_id', 'energy', 'danceability', 'valence', 'loudness'], model['audio_features']))
    seed_sections.extend(SQL.build_insert_batches('User_Behavior', ['user_id', 'song_id', 'play_count', 'liked', 'rating'], model['user_behavior']))
    seed_sections.extend(SQL.build_insert_batches('Song_Playlist', ['song_id', 'playlist_id'], model['song_playlist']))

    seed_sql = '\nGO\n'.join(seed_sections)
    schema_path.write_text(SCHEMA_SQL, encoding='utf-8')
    seed_path.write_text(seed_sql, encoding='utf-8')
    full_path.write_text(SCHEMA_SQL + '\n' + seed_sql, encoding='utf-8')

    print(f'Generated {schema_path.resolve()}')
    print(f'Generated {seed_path.resolve()}')
    print(f'Generated {full_path.resolve()}')
